> **SUPERSEDED IN PART (2026-09-23): the Dhesi statements in §0, §5 row 13, §5b and §6 item 5 are WRONG. Read "CORRECTION 2" at the end of this file and `DHESI_RECORD_RECONCILIATION_2026-09-23.md`. Original text is kept unedited below for the record.**
...
## CORRECTION 2 (2026-09-23, Agent 2): authoritative Dhesi interpretation (supersedes §0 row, §5 row 13, §5b, §6 item 5)
1. **The wrong-side stop was a real implementation bug** (5 of 30 v2 trades, each −$2 to −$10).
2. **The bug-only correction does NOT rescue v2.**
   - Removing the wrong-side trades alone gives −$411.88.
   - Side check with the code's continue rule gives −$304.88, and the holdout becomes −$0.62/trade (Agent 1).
3. **The −$446 → +$617 swing is NOT caused by the bug.** It comes from a post-hoc 10-point stop floor plus replacement trades that the guard's skip/continue behaviour enables. It rests on about 2–3 trades.
4. **The positive full v3 result is a NEW DEVELOPMENT-DERIVED HYPOTHESIS.** It is not a bug-corrected v2, and it has no uncontaminated evidence.
5. **2026-07-01 → 2026-09-18 is REUSED, not fresh.** At least 6 intraday-momentum trials and 3 order-flow trials read that CME index dataset group before v3. The "fresh" +$94.6/trade (n=4) is not evidence.
6. **Prior Dhesi state = INCONCLUSIVE, not robustly dead and not falsely killed.** MNQ Dhesi was never killed.
